...
Обязательные аргументы:
  -t, --target <IP>          Целевой IP-адрес (например, 89.23.123.4)
  -p, --port <ПРАВИЛО>       Порт или маппинг портов (можно указывать несколько раз)
                             Форматы:
                               2222:22            (TCP: внешний 2222 -> целевой 22)
                               25565              (TCP: 25565 -> целевой 25565)
                               24454:24454/udp    (UDP: 24454 -> целевой 24454)
...
Примеры:
  # Проброс SSH (2222 -> 22), Minecraft (25565) и VoiceChat (24454 UDP) на danki:
  sudo $0 -t 89.23.123.4 -p 2222:22 -p 25565 -p 24454/udp
...
if [[ ${#RAW_PORTS[@]} -eq 0 ]]; then
    echo -e "${RED}[ERROR] Не указан ни один порт (--port).${NC}" >&2
    usage 1
fi
