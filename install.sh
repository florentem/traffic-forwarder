...
Опциональные аргументы:
  -p, --port <ПРАВИЛО>       Порт или маппинг (по умолчанию: 25565/tcp и 24454/udp)
                             Форматы:
...
Примеры:
  # Проброс игрового порта (25565) и VoiceChat (24454 UDP) на danki:
  sudo $0 -t 89.23.123.4
...
# По умолчанию пробрасываем игровой порт (25565 TCP) и Voice Chat (24454 UDP)
if [[ ${#RAW_PORTS[@]} -eq 0 ]]; then
    echo -e "${CYAN}[INFO] Порты не указаны вручную. Используются стандартные игровые порты:${NC}"
    echo -e "${CYAN}       • 25565 (TCP)  — Minecraft (Velocity)${NC}"
    echo -e "${CYAN}       • 24454 (UDP)  — Simple Voice Chat${NC}"
    RAW_PORTS=("25565:25565/tcp" "24454:24454/udp")
fi
